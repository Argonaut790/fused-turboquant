...
def run_benchmark(
    ...
    if include_dejan:
        ...
        if make_quantized_cache is not None:
            methods[f"Dejan.ai TQ{bits}"] = lambda b=bits, f=make_quantized_cache: f(bits=b)
        else:
    ...
    for prompt_idx, prompt in enumerate(PROMPTS):
        ...
        for method_name, factory in methods.items():
            ...
            print(
                f"    {method_name}: {r['tokens_per_sec']:.1f} tok/s "
        ...
        unpatch_model(model)
        print(
            f"    {fused_label}: {r['tokens_per_sec']:.1f} tok/s "
            f"({r['gen_tokens']} tokens, {r['wall_time_s']:.2f}s, "
            f"peak={r['peak_memory_mb']:.0f} MB)"
        )
...
def main():
    ...
    parser.add_argument(
        "--bits", type=int, default=4, choices=[2, 3, 4], help="Quantization bit-width"
    )
    parser.add_argument(
        "--max-new-tokens", type=int, default=200, help="Max tokens to generate per prompt"
    )
